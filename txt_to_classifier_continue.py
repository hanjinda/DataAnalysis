...
file_to_form_basic_network = "network_basic.txt"
file_line_num = 1
...
f_out = open(file_out_dir + "network_basic.txt","w")
...
for num in range (file_start, file_end):
	...
	for num, line in enumerate(f):
		if num == 0:
			...
			# if line[1] != "":
			# 	f_out.write(str(file_line_num) +"\t")

		if "Definition: " in line:
			line = line.split("Definition: ")
			print line[1]
			f_out_2.write(str(file_line_num + 3910) + "\t" +line[1])
			if line[1] != "":
				f_out.write(str(file_line_num) +"\t" + str(file_line_num + 3910) +"\n")

		if "Symptoms and Signs: " in line:
			line = line.split("Symptoms and Signs: ")
			print line[1]
			f_out_3.write(str(file_line_num + 7871) + "\t" +line[1])
			if line[1] != "":
				f_out.write(str(file_line_num) +"\t" + str(file_line_num + 7871) +"\n")

		if "Treatment: " in line:
			line = line.split("Treatment: ")
			print line[1]
			f_out_4.write(str(file_line_num + 11360) + "\t" +line[1])
			if line[1] != "":
				f_out.write(str(file_line_num) +"\t" + str(file_line_num + 11360) +"\n")

		if "Causes: " in line:
			line = line.split("Causes: ")
			print line[1]
			f_out_5.write(str(file_line_num + 14728) + "\t" +line[1])
			if line[1] != "":
				f_out.write(str(file_line_num) +"\t" + str(file_line_num + 14728) +"\n")

		if "Diagnosis: " in line:
			line = line.split("Diagnosis: ")
			print line[1]
			f_out_6.write(str(file_line_num + 18062) + "\t" +line[1])
			if line[1] != "":
				f_out.write(str(file_line_num) +"\t" + str(file_line_num + 18062) +"\n")
